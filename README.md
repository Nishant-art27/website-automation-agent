...
An intelligent, **AI-driven website automation agent** that opens a browser,
navigates to a web page, **detects the form elements on its own**, and fills
them in **without any manual intervention**. It is a mini version of tools like
[Browser Use](https://github.com/browser-use/browser-use), built on
[Playwright](https://playwright.dev/).

> **Target task (from the assignment):** navigate to
> `https://ui.shadcn.com/docs/forms/react-hook-form`, identify the form fields
> (Name and Description), and fill them in automatically.

The live page actually labels its fields **“Bug Title”** and **“Description”**,
so the agent has to be *smart* about it — it maps the requested “Name” to the
first single-line text input and “Description” to the textarea. That element
detection is the interesting part, and it is handled two different ways:
...
Both brains drive the **same set of primitive tools** required by the
assignment — they only differ in *how* they choose coordinates. The heuristic
brain means **the demo always works, even with no API key and no internet model
access.**
...
- **All seven required tools**, implemented in `BrowserController`:
  ...
  `send_keys`, `scroll`, `double_click` (plus `press_key`).
- **Pluggable brain** — LLM when a key is present, heuristic fallback otherwise.
- **Intelligent element detection** via accessible labels, `aria-*`,
  placeholders, associated `<label>`s, control-type preference and positional
  fallback (`src/agents/matcher.ts`).
- **Self-verification** — after filling, the agent reads the live field values
  back and only reports success if they actually match.
- **Robust error handling** — navigation/timeout/element-not-found errors are
  caught, logged, and (for the LLM) fed back so it can recover.
- **Comprehensive logging** — colourised console output **and** a full
  JSON run log written to `artifacts/`.
- **Artifacts** — a screenshot is saved at every meaningful step, plus a
  machine-readable `artifacts/result.json`.
- **Config via env vars / CLI** — nothing is hard-coded.
...
### Run (no API key needed — uses the heuristic brain)
```bash
npm run agent
```
A Chromium window opens, the agent navigates to the shadcn form, fills in the
Name/Title and Description fields, and prints a result report. Screenshots and
`result.json` land in `./artifacts`.

### Run headless (e.g. on a server / CI)
```bash
HEADLESS=true npm run agent
```

### Run with the LLM brain
```bash
cp .env.example .env
# edit .env and set GROQ_API_KEY=gsk_...   (free key: https://console.groq.com/keys)
npm run agent
# or force it: npm run agent -- --mode llm
```
...
**With no options it runs the built-in test** (the shadcn form, default Name +
Description). Pass options to point it at any other form.
...
# 2) Explicit fields on any site (repeat --field). Works with both brains.
#    Labels are fuzzy-matched, so "Customer name" finds the right input.
...
All options also work as environment variables — see **`.env.example`** for the
...
full perceive → reason → act workflow.
...
- **Nothing happens / blank page** — the docs site may be slow; increase
  `DEFAULT_TIMEOUT_MS`. Re-run with `--log debug` to see every step.
- **LLM brain not used** — confirm `GROQ_API_KEY` is set, or force it with
  `--mode llm`.
- **Want to see it live in a viva** — run without `HEADLESS` and with
  `SLOW_MO_MS=400` so each action is easy to follow.
